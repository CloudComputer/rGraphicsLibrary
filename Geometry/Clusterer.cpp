...
PointCluster::PointCluster(){
	_points = new KDTree<NormalPoint,3,float>();
}
...
void PointCluster::addPoint(float point[3],NormalPoint np){
	_points->insert(point,np);
}
//
...
Clusterer::ClusterPoints(KDTree<NormalPoint,3,float> *_points,float d,unsigned int minSize){
	KDTree<NormalPoint,3,float>::NodeIterator point;
	std::vector<KDTree<NormalPoint,3,float>::Node*>::iterator it;

	KDTree<NormalPoint,3,float> tree;
	KDTree<NormalPoint,3,float>::Node* node;
	std::vector<KDTree<NormalPoint,3,float>::Node*> left;
	...
	while(!tree.empty()){
		...
		while(!left.empty()){
			...
			std::vector<KDTree<NormalPoint,3,float>::Node*> nodes = tree.findCloseTo(node->getPosition(),d);
