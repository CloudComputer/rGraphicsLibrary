...
ScalarField(v->getDimensions(),v->getBoundingAABB())
{
	...
	TmpPointer<ScalarField> v_blur = v->blur();DBG("v_blur");
	TmpPointer<VectorField> v_grad = v->getGradientField();DBG("v_grad");
	TmpPointer<VectorField> v_blur_grad = v_blur->getGradientField();DBG("v_blur_grad");
	TmpPointer<VectorField> curl = v_blur_grad->getCurlField();DBG("curl");

	auto dim = v->getDimensions();
	float size = dim.x * dim.y * dim.z;
	...
	FOR(dim){
		...
		auto V = v_blur_grad->_data[id];
		auto c = curl->_data[id];
